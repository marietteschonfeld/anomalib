...
class SPADEModel(nn.Module):
    ...
    def compute_patch_scores(self, embedding: torch.Tensor, top_K_images: torch.Tensor) -> torch.Tensor:
        ...
        embedding = embedding.permute(2,3,0,1).reshape(H*W,batch_size, embedding_size)
        bank = self.memory_bank.permute(2,3,0,1).reshape(H*W, self.memory_bank.shape[0], embedding_size)
        patch_scores,_ = torch.cdist(embedding, bank).min(dim=-1)

        patch_scores = patch_scores.reshape(H,W,batch_size).permute(2,0,1).unsqueeze(1)
        return patch_scores
    ...
    def compute_anomaly_scores(self, embedding: torch.Tensor) -> torch.Tensor:
        ...
        distances = torch.cdist(embedding.view(batch_size,-1), self.memory_bank.view(train_size, -1), compute_mode='use_mm_for_euclid_dist')
        pred_scores, top_K_images = torch.topk(distances, k=self.K_im, dim=1)
        return pred_scores.sum(dim=1), top_K_images
